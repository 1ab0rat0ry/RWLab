...
function DakoBs2:updateControlMechanism(timeDelta, position, feedPipe, brakePipe)
    if position <= self.ranges.RELEASE then
        ...
        self.interruptValve = MathUtil.towards(self.interruptValve, 1, 2 * timeDelta)
        self.releaseValve = true
    elseif position <= self.ranges.RUNNING then
    --fill brake pipe through choked connection
        self.emergencyValve = false
        self.interruptValve = MathUtil.towards(self.interruptValve, 0.3, 2 * timeDelta)
        self.releaseValve = false
        self.setPressure = REFERENCE_PRESSURE
    elseif position <= self.ranges.NEUTRAL then
        --isolate brake pipe
        self.emergencyValve = false
        self.interruptValve = MathUtil.towards(self.interruptValve, 0, 2 * timeDelta)
        self.releaseValve = false
